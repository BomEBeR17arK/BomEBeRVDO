...
/**
 * Step 5.1: rows for a downloadable item (same set as isPlayable — DIRECT_FILE/HLS)
 * now also show a "ดาวน์โหลด" action next to "เล่น", calling the new
 * [onDownloadMedia] param (wired in HomeStreamNavHost to DownloadRepository.startDownload).
 * Everything else is unchanged from Step 4.
 */
@Composable
fun BrowserScreen(
...
/** Same gate as Step 4's Play button — Step 5.1 only downloads what PlayerScreen can already play. */
private fun isActionable(accessType: MediaAccessType): Boolean =
...
private fun DetectedMediaRow(
    ...
    Column(modifier = Modifier.padding(vertical = 6.dp)) {
        Row(
            ...
            if (isActionable(media.accessType)) {
                Row {
